...
import com.enfore.apis.ast.SwaggerAST.{Component, CoreASTRepr, MediaTypeObject, ParamObject, PathObject, Property, SchemaObject, SchemaRefContainer, SchemaStore}
    ...
    SchemaStore(
      Map(
        "Contact" -> Component(
          ...
          Some(
            Map(
              "id" -> Property(Some("The technical identifier of the contact. Assigned by the enfore platform on creation of the contact and not changeable afterwards."), Some(string), None, None, Some(true), None, None),
              "type" -> Property(None, Some(string), None, None, None, None, None),
              "name" -> Property(Some("The name of the contact.\nFor individual contacts, this is the full name of the person, e.g., \"Barnabas Ludwig Johnson II Sr.\".\n" +
        ...
        "IndividualContact" -> Component(
          ...
          None,
          None
        ), "Gender" -> Component(Some("The gender of a human person"),componentString,None,Some(List("FEMALE", "MALE")),None)
      )),
    Some(
      Map("/contacts/individual/{contact-id}" -> Map(
        "get" -> PathObject(
